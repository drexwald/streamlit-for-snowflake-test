...
def get_system_prompt():
    ...
    # get metadata
    sql = ("SELECT P_NAME, P_TYPE"
        + f" FROM  {DATABASE}.{SCHEMA}.PART;")
    metadata = conn.query(sql, show_spinner=False)
    metadata = "\n".join([
        f"- **{metadata['P_NAME'][i]}**: {metadata['P_TYPE'][i]}"
        for i in range(len(metadata["P_NAME"]))])
    ...
    \nHere is the table name <tableName> {DATABASE}.{SCHEMA}.{TABLE}

    \n\nAvailable variable by VARIABLE_NAME:\n\n{metadata}
